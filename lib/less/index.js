...
module.exports = {
	...
	doSomething: function(master) {
		...
		var css = keys.map(function(key) {
			...
			var before = object.before ? object.before : (function(value) { return value })
			var after = object.after ? object.after : (function(value) { return value })

			var sources = object.files.map(function(file) {
				return before(fs.readFileSync(file, 'utf8'))
			})

			var blocks = sources.map(function(source, index) {
				return after(object.files[index], this.render(source, {}))
			}.bind(this))
